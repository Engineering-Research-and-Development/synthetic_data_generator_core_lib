...
def ctgan_model(generator, critic):
    return CTGANModel(
        generator=generator, critic=critic, onehot_discrete_indexes=[2, 3, 4]
    )
...
def test_generator_call(generator):
    batch_size = 4
    noise_dim = (
        4  # skeleton has 2 continuous + 2 continuous = 4 total continuous features
    )
...
def test_ctgan_model_generate_without_fitting(ctgan_model):
    ...
    assert "In order to generate some data you need to fit a dataset first!" in str(
        exception_info
    )
