...
correct_skeleton = json.load(open(os.path.join(current_folder, "correct_skeleton.json")))
...
def test_table_get_computing_data(temp_folder):
    ...
    assert np.all(data == np.array([[1, 2, 3, 4, 5, 6], ["a", "b", "c", "d", "e", "f"]]).T)
...
def test_postprocess_continuous_column(temp_folder):
    ...
    assert np.all(postprocessed_table.get_computing_data() == np.array([1, 2, 3]).reshape(-1,1))
...
def test_preprocess_categorical_column(temp_folder):
    ...
    assert np.all(processed_table.get_computing_data() == np.array([[1., 0., 0.], [0., 1., 0.], [0., 0., 1.]]))
...
def test_postprocess_categorical_column(temp_folder):
    ...
    assert np.all(postprocessed_table.get_computing_data() == np.array(["a", "b", "c"]).reshape(-1,1))
